      ...
      subsections: [
        {
          title: "0. 검 기본기",
          content:
            "모든 검술 투로의 출발점이에요. 세우기·베기·자세·보법의 토대를 세워, 이후 투로 수련에서 기술이 자연스럽게 흐를 수 있도록 합니다.",
        },
        {
          title: "검술 1트랙",
          content:
    ...
    <MainLayout>
      <div className="container mx-auto px-4 py-6 sm:py-8">
        <motion.div initial={false} animate={{ opacity: 1, y: 0 }}>
          <PageHeading size="lg">{turoData.title}</PageHeading>
        </motion.div>
        ...
        >
          <CardContainer>
            <div className="flex flex-col md:flex-row items-start gap-6 sm:gap-8">
              ...
              <div className="md:w-1/2">
                <Body size="base">{turoData.overview.description}</Body>
              </div>
            </div>
          </CardContainer>
        </motion.section>
                ...
                {isProse ? (
                  <ProseContainer className="text-center">
                    ...
                    {section.subsections ? (
                      ...
                      <Body size="base">{section.content}</Body>
                    )}
                  </ProseContainer>
                ) : (
                  <CardContainer>
                    ...
                    {section.subsections ? (
                      <div className="grid md:grid-cols-2 gap-6">
                        {section.subsections.map((sub, idx) => (
                          <motion.div
                            ...
                            className="space-y-3"
                          >
                            <SubHeading size="md">{sub.title}</SubHeading>
                            <Body size="sm">{sub.content}</Body>
                            {sub.image && (
                              <div>
                                <Image
                                  src={sub.image}
                      ...
                      <Body size="sm">{section.content}</Body>
                    )}
                  </CardContainer>
                )}
              </motion.div>
