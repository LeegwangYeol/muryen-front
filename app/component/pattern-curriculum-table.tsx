...
/**
 * 투로 수련 과정 시각화 테이블.
 * 기존 본문에 병기된 4개 카드(기본기/1트랙/2트랙/기타무기)의
 * 순차·병렬 관계를 한눈에 보이도록 정리한 보조 자료.
...
export default function PatternCurriculumTable() {
  ...
  const textBody = theme === "dark" ? "text-white/90" : "text-gray-800";
  ...
  return (
    <section className={`p-5 sm:p-8 rounded-lg border mt-8 sm:mt-12 ${glass} ${border}`}>
      <div className="flex items-baseline justify-between mb-4 sm:mb-6 gap-3 flex-wrap">
        <h3 className={`text-xl sm:text-2xl font-bold ${textHeading}`}>
          커리큘럼 한눈에 보기
        </h3>
